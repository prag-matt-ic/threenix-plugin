...
description: Add Threenix cinematic camera controls to an existing React Three Fiber scene. Use when the user invokes $add-camera-controls or asks for distance-aware camera transitions with optional pointer look or lens zoom.
---
...
3. Call the `get_component_reference` tool from the `threenix` MCP server with `{ "slug": "camera-controls" }`. If the tool or server is unavailable, stop and explain the problem; do not reconstruct the reference from local or bundled sources.
...
5. Follow the returned `integrationNotes`. Call `useCameraControls` from a component inside the existing Canvas and wire only the requested `setLookAt`, `setZoom`, pointer, or distance behavior. Drive zoom with `setZoom` and leave `camera.zoom` and `camera.fov` to the hook. Preserve the target's Canvas, renderer, scene, postprocessing, and UI; do not copy demo shots, clipping logic, Leva controls, or backgrounds.
