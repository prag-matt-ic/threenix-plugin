...
3. Call the `get_component_reference` tool from the `threenix` MCP server with `{ "slug": "fast-text" }`. If the tool or server is unavailable, stop and explain the problem; do not reconstruct the reference from local or bundled sources.
