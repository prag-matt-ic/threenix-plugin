...
3. Call the `get_component_reference` tool from the `threenix` MCP server with `{ "slug": "scene-warmup" }`. If the tool or server is unavailable, explain the access problem; do not reconstruct the reference from local or bundled sources.
